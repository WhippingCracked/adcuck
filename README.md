...
`feed/discovered.json`, ready to merge into `filters.js`.

It reads only and applies nothing. Every candidate is a guess until you have
looked at it: the pattern that catches `promotedVideoRenderer` also catches
things with "ad" in the middle of an innocent word, and a filter list that eats
real content is worse than one that misses an ad. Run it monthly, or whenever
something starts leaking through.
