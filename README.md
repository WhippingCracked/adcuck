...
| File | What it does |
|---|---|
| `1-get-filters.bat` | Opens YouTube, finds ads you are not blocking yet, then asks you about each one and adds the ones you approve to `filters.js` |
...
```bash
node tools/discover.mjs                 # a few default pages
node tools/discover.mjs <url> <url> …   # or your own
```
...
it saw that the current list does **not** cover — ranked by how often each
appeared, with a sample of each so you can see what it actually is. It writes
`feed/discovered.json` and applies nothing.

Every candidate is a guess until you have looked at it: the pattern that
catches `promotedVideoRenderer` also catches things with "ad" in the middle of
an innocent word, and a filter list that eats real content is worse than one
that misses an ad. Run it monthly, or whenever something starts leaking
through.

### Reviewing what it found

```bash
npm run add            # or: node tools/add-filters.mjs
```

Reads `feed/discovered.json`, skips anything already covered, and shows the
rest one at a time — `y` to block it, `n` to leave it, `q` to stop. What you
approve is written into the right list in `src/filters/filters.js`, keeping
that file's existing indentation.
...
tools/
  discover.mjs               finds ad-shaped things not yet blocked
  add-filters.mjs            reviews them one by one, edits filters.js
